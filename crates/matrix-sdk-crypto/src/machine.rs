...
pub(crate) mod test {
    ...
    async fn interactive_verification_started_from_request() {
        ...
        let event = request_to_event(bob.user_id(), &accept_request.into());
        alice.handle_verification_event(&event).await;
        ...
        let event = request_to_event(bob.user_id(), &start_request_from_bob.into());
        alice.handle_verification_event(&event).await;

        // Since Alice's user id is lexicographically smaller than Bob's, Alice does not do
        // anything with the request, however.
        ...
        let event = request_to_event(alice.user_id(), &start_request_from_alice.into());
        bob.handle_verification_event(&event).await;
