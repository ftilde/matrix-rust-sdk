...
impl SasState<Started> {
    ...
    /// an Accepted one.
    ///
    /// Note: Even though the other side has started the (or rather "a") sas verification, it can
    /// still accept one, if we have sent one simultaneously. In this case we just go on with the
    /// verification that *we* started.
    ///
    /// # Arguments
    ///
    /// * `event` - The m.key.verification.accept event that was sent to us by
